...
Reseller-managed gateways support **automatic fee collection** — the platform deducts [reseller fees](/topics/reseller-fees) at the point of payment rather than settling them separately after the fact. When a customer pays, the gateway splits the transaction so that the reseller's fee is collected automatically and the company receives the remainder.

This works because the reseller holds the master gateway credentials and each company operates as a sub-account within the gateway. The exact mechanism differs by provider:

| Gateway | Fee mechanism | Sub-account model |
|---------|--------------|-------------------|
...
| **Curlec** | Transfer-based fee deduction | Company connects as a Razorpay Linked Account |
| **Nuvei** | Platform fee collection | Company operates under reseller's merchant configuration |

For all five gateways, the behaviour is the same from a reseller's perspective: fees configured in [reseller fees](/topics/reseller-fees) are collected automatically on every transaction. No manual settlement or reconciliation is required for the fee portion.

### Refund Behaviour

When a refund is processed, gateways handle the previously collected fee differently:

- **Stripe** — by default, the application fee is reversed proportionally when a partial refund is issued, and fully reversed on a full refund.
...
- **PayPal, Curlec, Nuvei** — fee reversal behaviour follows each provider's default refund policy.

### Company-Managed Gateways

Gateways where the company provides its own credentials (e.g. Adyen, QuickPay, Worldpay) do **not** support automatic fee collection. For these gateways, the reseller's fee is tracked by the platform and settled separately through [company invoices](/topics/company-invoices).
